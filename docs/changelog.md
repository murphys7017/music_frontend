...
我需要你在page下面创建一个home文件夹，里面主要有两个文件，一个home page，一个home controller，home page主要负责画页面，controller负责提供控制逻辑等，同时我对页面的布局设想如图。我希望的逻辑就是每日的第一次发起请求32首常听，其中30首放歌单，最常听的2首放外面。请求31首不常听，一首放外面，剩下的放歌单。之后的没错请求只请求两首常听一首不常听更新外面单曲，不更新歌单

我希望再添加两个歌单
1.猜你喜欢
2.好久不见
这两个歌单每天更新一次，请求服务器端的两个推荐接口跟新